...
def create_retriever(index: VectorStoreIndex, top_k_retriever: int, cutoff=None, alpha=0.5, filters=None) -> VectorIndexRetriever:
...
def get_llm_answer(index, llm, tag, args, query_list=None):
    filters = None
    #filters = MetadataFilters(filters=[MetadataFilter(key="Tag", value=tag)], condition="or")

    retriever = create_retriever(
        index, cutoff=args.cutoff, top_k_retriever=args.top_k_retriever, filters=filters
    )
...
if __name__ == "__main__":
    ...
    if args.folder:
        ...
        get_llm_answer(index, llm, tag, args, query_list)
    else:
        for tag in all_tags:
            print("\n\nApply query to " + tag + " folder only")
            get_llm_answer(index, llm, tag, args, query_list)
